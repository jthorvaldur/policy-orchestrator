...
Operating rules for all repos live in the global agent instructions (`~/.claude/CLAUDE.md`, "Operating Rules"): signal over noise, no drift, flag uncertainty, simple > complex, explicit > implicit, auditable > opaque, reversible > permanent. Hard policies in `policies/hard/` enforce them; agent contracts live in `registries/agents.yaml`.
...
1. Identify the relevant repo intent before producing output.
